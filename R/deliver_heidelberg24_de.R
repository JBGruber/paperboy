#' @export
pb_deliver_paper.heidelberg24_de <- function(x, verbose = NULL, pb, ...) {
    ...
    if (isTRUE(is.na(json_txt)) || length(json_txt) == 0) {
        ...
        text <- html %>%
            rvest::html_elements(".id-StoryElement-leadText,.id-StoryElement-paragraph,.id-StoryElement-crosshead") %>%
            rvest::html_text2() %>%
            paste(collapse = "\n")
