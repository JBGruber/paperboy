...
Use the tools you are most comfortable with, I usually just right-click and inspect in chrome and then look for a suitable CSS tag.
...
To make the process even easier, `paperboy` adds the function `html_search` which can search through multiple CSS indetifiers and node attributes at once.
...
It looks through all the listed selectors and retrieves the attribute content or the text of the node if anything is found.

The most difficult decision for most parsers is which CSS tags to choose.
There is usually a trade-off between making the parser more specific and have clean data or make it more general to have less trouble maintaining different formats on the same site.
...
This selects all paragraphs on a site, which often also includes irrelevant data, such as infos for subscribers, ads or even readers' comments.
So it is usably suggested to do something like this instead:
...
`html_search()` can do this:

```{r}
text3 <- html %>%
  html_search(selectors = c(".article-body>p", "p"), 
              attributes = "text", 
...
You should still make sure though that the objects headline, article, datetime, and author have lenght 1 (with `toString()` and `head(x, 1L)` respectively)
